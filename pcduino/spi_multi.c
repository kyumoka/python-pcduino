 ...
 * Copyright (c) 2015  kyumoka <qmh524@qq.com>
 *
...
static void set_spi_mode(int gpio, char mux) {
	char path[100];
	sprintf(path, "/sys/devices/virtual/misc/gpio/mode/gpio%d", gpio);
	int fd = open(path, O_RDWR);
	if (fd < 0)
		pabort("can't set pin mode");
	if (write(fd, &mux, 1) <= 0)
		pabort("can't set pin mode");
	close(fd);
}
...
static PyObject* openSPI(PyObject *self, PyObject *args, PyObject *kwargs) {
	...
	if (strcmp(device, "/dev/spidev0.0") == 0) {
		// init default pin mode
		set_spi_mode(10, '2');
		set_spi_mode(11, '2');
		set_spi_mode(12, '2');
		set_spi_mode(13, '2');
	} else if (strcmp(device, "/dev/spidev2.0") == 0) {
		set_spi_mode(20, '3');
		set_spi_mode(21, '3');
		set_spi_mode(22, '3');
		set_spi_mode(23, '3');
...
static PyObject* transfer(PyObject* self, PyObject* arg) {
	...
	if(!PyArg_ParseTuple(arg, "OO", &param, &transferTuple))
		return NULL;
	if (!PyDict_Check(param))
		pabort("dict param not found\n");

	int fd = PyInt_AsLong(PyDict_GetItemString(param, "fd"));
	uint8_t bits = PyInt_AsLong(PyDict_GetItemString(param, "bits"));
	uint32_t speed = PyInt_AsLong(PyDict_GetItemString(param, "speed"));
	uint16_t delay = PyInt_AsLong(PyDict_GetItemString(param, "delay"));
	...
	uint8_t *tx = malloc(sizeof(uint8_t) * tupleSize);
	uint8_t *rx = malloc(sizeof(uint8_t) * tupleSize);
	PyObject* tempItem;
...
static PyObject* closeSPI (PyObject* self, PyObject* arg) {
	...
	if(!PyArg_ParseTuple(arg, "O", &param))
		return NULL;
